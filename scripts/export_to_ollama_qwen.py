...
Run:
    ...
    python scripts/export_to_ollama_qwen.py --quant Q5_K_M
"""
...
# exactly -- the same one train_qlora_qwen.py applied via get_chat_template(..., "qwen-2.5").
QWEN25_TEMPLATE = (
...
def main() -> None:
    ...
    if not (args.adapter_dir / "adapter_config.json").exists():
        raise SystemExit(f"no adapter at {args.adapter_dir} -- run train_qlora_qwen.py first")
    try:
